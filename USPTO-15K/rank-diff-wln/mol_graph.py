...
def smiles2graph(rsmiles, core_bonds, gold_bonds, cutoff=500, idxfunc=lambda x:x.GetIntProp('molAtomMapNumber') - 1):
    ...
    #Compute the golden label
    for x,y,t in gold_bonds:
        gbonds[(x,y)] = t
    gold_bonds = set()
    for x,y in core_bonds:
        t = gbonds[(x,y)] if (x,y) in gbonds else 0
    ...
    #New Rings
    new_rings = find_rings(mol, rbonds, core_bonds)
    for ring in new_rings:
        ...
        cur_id = -1
        for x,y in ring_edges:
            if (x,y) in core_bonds:
                cur_id = comp[core_bonds.index((x,y))]
                break
...
if __name__ == "__main__":
    import sys
    train_f = open("future/aromatic.txt", 'r')
    ...
    for line in train_f:
        ...
        cand_bonds = []
        cbonds = list(set(cbonds))
        for b in cands.strip("\r\n ").split(','):
            x,y = b.split('-')
            x,y = int(x)-1,int(y)-1
